...
💡 提示: Mock 函数位于 .data/plans/{planId}/mocks/ 目录
   每个 plan 的 mock 函数独立存储，支持版本迭代
   你可以编辑这些文件来实现真实逻辑
```
...
1. 🔍 识别缺失的函数（如 `sqrt`）
2. 🤖 自动生成 mock 实现
3. 💾 保存到 `.data/plans/{planId}/mocks/` 目录（版本化存储）
4. ✅ 生成可执行的计划

生成的 mock 文件：
- 存储在 Plan 专属目录，跟随 Plan 生命周期
- 支持版本迭代（v1 → v2 → v3...）
- 可供后续完善（"悬赏模式"）
...
├── functions/             # 函数定义
│   ├── math.ts            # 数学函数（加减乘除）
│   └── generated/         # 遗留：兼容旧版本 Mock
├── __tests__/             # 端到端测试
├── .data/                 # 本地数据存储
│   └── plans/             # 执行计划 + Mock 函数
│       ├── plan-abc123/   # Plan 专属目录
│       │   ├── v1.json    # Plan 版本 1
│       │   ├── v2.json    # Plan 版本 2
